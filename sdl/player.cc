...
float f_reach_range(const t_vector3f& a_ball, const t_vector3f& a_velocity, const t_vector3f& a_player, float a_speed, float a_t0, float a_sign)
{
	...
	qp.v_y = 0.0f;
	t_vector3f v(a_velocity.v_x, 0.0f, a_velocity.v_z);
	float ss = a_speed * a_speed;
	float a = v * v - ss;
	float b = v * qp - ss * a_t0;
	float c = qp * qp - ss * a_t0 * a_t0;
	float d = b * b - a * c;
	return d < 0.0f ? -b / a : (-b + a_sign * sqrt(d)) / a;
}

t_vector3f f_shot_direction(const t_vector3f& a_ball, float a_end, bool a_left, bool a_right, bool a_forward, bool a_backward)
{
	float vx = -a_ball.v_x;
	if (a_left) vx -= 12 * 12 * 0.0254f * a_end;
	if (a_right) vx += 12 * 12 * 0.0254f * a_end;
	float vz = -24 * 12 * 0.0254f * a_end - a_ball.v_z;
	if (a_forward) vz -= 16 * 12 * 0.0254f * a_end;
	if (a_backward) vz += 10 * 12 * 0.0254f * a_end;
	return t_vector3f(vx, 0.0f, vz);
}
...
void t_player::t_swing::f_initialize(t_document& a_scene, t_node* a_skeleton, float a_start, float a_duration, float a_impact, float a_speed, const t_vector3f& a_spin)
{
	...
	auto iterators = a_scene.f_iterators();
	{
		...
		a_skeleton->f_render(t_matrix4f(1.0f), t_matrix4f(1.0f), js);
	}
	{
		...
		std::vector<std::map<std::wstring, t_matrix4f*>*> js{&joints};
		a_skeleton->f_render(t_matrix4f(1.0f), t_matrix4f(1.0f), js);
		v_end_position = t_vector3f(root[3][0], 0.0f, root[3][2]);
		v_end_toward = t_vector3f(root[2][0], 0.0f, root[2][2]);
	}
}
...
void t_player::t_run::f_initialize(t_document& a_scene, t_node* a_skeleton, float a_start, float a_duration, const std::function<bool (const t_channel&)>& a_use)
{
	...
	std::vector<std::map<std::wstring, t_matrix4f*>*> js{&joints};
	a_skeleton->f_render(t_matrix4f(1.0f), t_matrix4f(1.0f), js);
	v_toward = t_vector3f(root[2][0], 0.0f, root[2][2]);
}

void t_player::t_motion::operator()()
{
	v_action.f_forward(v_time);
	if (v_time < v_action.v_end) v_time += 1.0f / 60.0f;
}
...
t_player::t_run_motion::t_run_motion(t_run& a_run, const t_vector3f& a_toward, t_player& a_player) : t_motion(a_run), v_toward(a_toward), v_placement(a_player.v_placement), v_toward0(a_player.v_placement->v_toward)
{
	...
	v_toward1 = t_vector3f(t0.v_x * t1.v_z + t0.v_z * t1.v_x, 0.0f, t0.v_z * t1.v_z - t0.v_x * t1.v_x);
	v_duration = v_toward0 * v_toward1 < -0.75f ? 0.0f : c_duration;
}

void t_player::t_run_motion::operator()()
{
	t_motion::operator()();
	if (v_duration > 0.0f) v_duration -= 1.0f / 64.0f;
	float t = v_duration / c_duration;
	v_placement->v_toward = v_toward0 * t + v_toward1 * (1.0f - t);
}
...
const t_player::t_state t_player::v_state_default{
	...
	[](t_player& a_player)
	{
		...
		auto f = [&](auto& actions, auto run_done, auto run_default, auto run_takeback)
		{
			if (a_player.v_ball.v_done) {
				run_done(t_vector3f(0.0f, 0.0f, -a_player.v_end), actions.v_default);
				...
				float t = f_reach_range(a_player.v_ball.v_position, a_player.v_ball.v_velocity, a_player.v_placement->v_position, 0.0f, 0.0f, 1.0f);
				float y = a_player.v_ball.v_position.v_y + (a_player.v_ball.v_velocity.v_y - 0.5f * G * t) * t;
				if (y > a_player.f_smash_height()) {
					...
					run_takeback(v, hand, a_player.v_ball.v_in || y < 0.0f ? hand.v_stroke : hand.v_volley);
				}
			}
		};
		if (d.v_x == 0.0f && d.v_z == 0.0f) {
			...
			}, [&](auto v, auto& actions, auto& action)
			{
				if (a_player.v_forward)
					...
					g(action, v.v_x * a_player.v_end > 0.0f ? actions.v_forehand.v_lowers[9] : actions.v_backhand.v_lowers[10]);
				else
					g(action, a_player.v_actions.v_run.v_lower);
			}, [&](auto v, auto& hand, auto& action)
			{
	...
	[](t_player& a_player, t_swing t_shots::* a_shot)
	{
		...
		float t = a_player.v_ball.f_projected_time_for_y(a_player.f_smash_height(), 1.0f);
		if (!isnan(t)) {
...
const t_player::t_state t_player::v_state_serve_set{
	...
	[](t_player& a_player)
	{
		...
		const float center = 12 * 0.0254f;
		const float wide = 14 * 12 * 0.0254f;
		if (xes < center) a_player.v_ball.v_position.v_x = center * es;
		if (xes > wide) a_player.v_ball.v_position.v_x = wide * es;
		a_player.v_ball.v_position.v_y = 0.875f;
		a_player.v_ball.v_velocity = t_vector3f(0.0f, 0.0f, 0.0f);
		a_player.v_ball.v_spin = t_vector3f(0.0f, 0.0f, 0.0f);
		a_player.v_placement->v_position = t_vector3f(a_player.v_ball.v_position.v_x, 0.0f, a_player.v_ball.v_position.v_z);
		a_player.v_placement->v_toward = t_vector3f((6 * 12 + 9) * -0.0254f * es + 2 * 12 * 0.0254f * a_player.v_lefty * a_player.v_end, 0.0f, 21 * 12 * -0.0254f * a_player.v_end) - a_player.v_placement->v_position;
		a_player.v_placement->v_valid = false;
		(*a_player.v_motion)();
	},
	[](t_player& a_player, t_swing t_shots::* a_shot)
...
const t_player::t_state t_player::v_state_serve_toss{
	[](t_player& a_player)
	{
		a_player.v_ball.v_position.v_y = 1.5f;
		a_player.v_placement->f_validate();
		const auto& toward = a_player.v_placement->v_toward;
		t_vector3f left(toward.v_z, 0.0f, -toward.v_x);
		a_player.v_ball.v_velocity = left * 0.0075f * a_player.v_lefty + toward * 0.01f;
		a_player.v_ball.v_velocity.v_y = 0.085f;
		a_player.v_ball.v_spin = t_vector3f(0.0f, 0.0f, 0.0f);
		a_player.v_motion = std::make_unique<t_motion>(a_player.v_actions.v_serve.v_toss);
	},
	[](t_player& a_player)
	{
		if (a_player.v_ball.v_position.v_y <= 1.5f) {
		...
		if (a_player.v_left) a_player.v_placement->v_toward.v_x -= 1.0f / 64.0f * a_player.v_end;
		if (a_player.v_right) a_player.v_placement->v_toward.v_x += 1.0f / 64.0f * a_player.v_end;
		a_player.v_placement->v_valid = false;
		(*a_player.v_motion)();
	},
	[](t_player& a_player, t_swing t_shots::* a_shot)
...
const t_player::t_state t_player::v_state_serve_swing{
	...
	[](t_player& a_player)
	{
		...
		if (fabs(a_player.v_motion->v_time - action.v_impact) < 0.5f / 60.0f) {
			auto ball = a_player.f_relative_ball(action);
			if (fabs(ball.v_y) < 0.3f) {
				float d = 58 * 12 * 0.0254f + ball.v_y * 10.0f;
				const auto& spin = action.v_spin;
				d *= pow(2.0f, -spin.v_x * (4.0f / 64.0f));
				float speed = action.v_speed + ball.v_y * 0.125f;
				a_player.v_ball.f_impact(a_player.v_placement->v_toward.v_x, a_player.v_placement->v_toward.v_z, speed, G * d / (2.0f * speed) - a_player.v_ball.v_position.v_y * speed / d, spin);
...
void t_player::f_swing_impact(const t_vector3f& a_v)
{
	...
	if (fabs(v_motion->v_time - action.v_impact) < 0.5f / 60.0f) {
		auto ball = f_relative_ball(action);
		if (fabs(ball.v_x) < 0.5f && ball.v_y < 1.0f && fabs(ball.v_z) < 1.0f) {
			float d = a_v.f_length();
			float n = -d * v_ball.v_position.v_z / a_v.v_z;
			float b = v_ball.v_position.v_y * (d - n) / d;
			float a = d / (60 * 12 * 0.0254f);
			float speed = action.v_speed * (a > 1.25f ? 1.25f : a < 0.85f ? 0.85f : a);
			const auto& spin = action.v_spin;
			float dd = v_ball.v_position.v_z * v_end;
			d = dd + (d - dd) * pow(2.0f, -spin.v_x * ((spin.v_x > 0.0f ? 12.0f : 8.0f) / 64.0f));
			float nh = (36 + 42) * 0.5f * 0.0254f + t_ball::c_radius;
			if (b < nh) {
				float vm = sqrt(G * (d - n) * n * 0.5f / (nh - b));
				if (vm < speed) speed = vm;
			}
			d -= ball.v_x * 2.0f;
			speed -= ball.v_x * 0.125f;
			float dx = a_v.v_x + a_v.v_z * ball.v_z * 0.0625f;
			float dz = a_v.v_z - a_v.v_x * ball.v_z * 0.0625f;
			v_ball.f_impact(dx, dz, speed, G * d / (2.0f * speed) - v_ball.v_position.v_y * speed / d, spin);
			v_ball.f_hit(this);
...
const t_player::t_state t_player::v_state_smash_swing{
	...
	[](t_player& a_player)
	{
		...
		if (fabs(a_player.v_motion->v_time - action.v_impact) < 0.5f / 60.0f) {
			auto ball = a_player.f_relative_ball(action);
			if (fabs(ball.v_x) < 0.5f && fabs(ball.v_y) < 1.0f && fabs(ball.v_z) < 1.0f) {
				float d = v.f_length() + (ball.v_y - ball.v_z) * 2.0f;
				float speed = action.v_speed + ball.v_y * 0.125f;
				float dx = v.v_x + v.v_z * ball.v_x * 0.0625f;
				float dz = v.v_z - v.v_x * ball.v_x * 0.0625f;
				a_player.v_ball.f_impact(dx, dz, speed, G * d / (2.0f * speed) - a_player.v_ball.v_position.v_y * speed / d, action.v_spin);
...
const t_player::t_state t_player::v_state_reach_swing{
	...
	{
		const auto& action = static_cast<t_swing&>(a_player.v_motion->v_action);
		float impact = (action.v_impact - a_player.v_motion->v_time) * 60.0f;
		...
		a_player.v_placement->v_toward = t_vector3f(vx, 0.0f, vz);
		a_player.v_placement->v_valid = false;
		a_player.v_stage.v_sound_swing.f_play();
	},
	[](t_player& a_player)
